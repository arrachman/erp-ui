...
class CRUD extends React.Component {
  constructor(props) {
    ...
    this.setTextInputRef = (element) => {
      if (element) { refData[element.id] = element; }
    };
    refData.filter = {};
    this.setRefFilter = (element) => {
      if (element) { refData.filter[element.id] = element; }
    };
  }
  ...
  handlers = {
    ...
    ctrl_b: event => {
      const {
        addNew, closeForm, dataTable, openForm
      } = this.props;
      if (!openForm) {
        addNew(dataTable);
      } else {
  ...
  componentDidUpdate(prevProps) {
    const {
      editingId, params, openForm, openFormFilter
    } = this.props;
  ...
  render() {
    const {
      params, openFormFilter, closeFormFilter, addNewFilter, submitForm, last_page, isSubmitting, editingId,
      addNew, closeForm, removeRow, editRow, dataTable, openForm, initValues, closeNotif, messageNotif,
      showSearchField, isFocusedField, valSearchField, setSearchGlobal
    } = this.props;
...
const mapDispatchToProps = dispatch => ({
  fetchData: (pagging) => dispatch({
    type: `${branch}/GET_DATA`, branch, pagging, source, primaryKey
  }),
  submitForm: (newData, editingId) => dispatch({
    type: `${branch}/SUBMIT_DATA`, branch, newData, editingId, source, primaryKey
  }),
  ...
  removeRow: (item) => dispatch({
    type: `${branch}/REMOVE_ROW_FORM`, branch, item, pagging: { page, limit }, source, primaryKey
  }),
  ...
  setSearchGlobal: (showSearchField, isFocusedField, valSearchField) => dispatch({
    type: `${branch}/SET_SEARCH_GLOBAL`, branch, showSearchField, isFocusedField, valSearchField
  }),
});
